...
After direction selection, select one claimant by request creation tick, then pre-intersection road-cell coordinate (row, column), then vehicle.id. Only that claimant receives the committed grant and reserves the downstream entry cell.
