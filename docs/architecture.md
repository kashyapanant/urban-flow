...
3. Count each demand attempt once: reject an active network that made no movement; otherwise reject at the type-specific active cap; otherwise proceed to origin, path, and transactional-admission checks. A failure of those checks is a no-admissible-entry rejection.
